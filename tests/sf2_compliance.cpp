...
namespace {
    ...
    void TestSf2SpecCompatibilityModeUsesResolverOwnedEffectsSend() {
        ...
        Require(NearlyEqual(voice.reverbSend, 0.25f, 1.0e-6),
            "SF2_SPEC_204 mode should keep reverb send at the spec-resolved preset value");
    ...
    void TestSf2SpecResolverEffectsSendUsesResolverValue() {
        ...
        std::vector<ResolvedZone> zones;
        const ResolvedZone& zone = RequireSingleZone(sf2, 60, 65535, &ctx, zones);
        const i32 expectedReverbGen = static_cast<i32>(std::lround(
            200.0 * Sf2SpecNormalize7Bit(ctx.ccValues[91])));
        const i32 expectedChorusGen = static_cast<i32>(std::lround(
            200.0 * Sf2SpecNormalize7Bit(ctx.ccValues[93])));
        Require(zone.generators[GEN_ReverbEffectsSend] == expectedReverbGen,
            "SF2 spec resolver should map CC91 into ReverbEffectsSend");
        Require(zone.generators[GEN_ChorusEffectsSend] == expectedChorusGen,
            "SF2 spec resolver should map CC93 into ChorusEffectsSend");

        Voice voice;
        voice.NoteOn(zone, sf2.SampleData(), sf2.SampleData24(), sf2.SampleDataCount(),
            0, 0, 0, 60, 65535, 1, 44100, 0.0, SoundBankKind::Sf2, compat);
        voice.UpdateChannelMix(1.0f, 0x80000000u, FloatToU32(0.5f), FloatToU32(0.25f));
    ...
    void TestSf2MixOnlyRefreshPreservesPitchAndFilterState() {
        ...
        voice.RefreshResolvedZoneControllers(
            refreshed, static_cast<u8>(Sf2ModulatorDestinationClassMask::Mix));

        Require(voice.baseGainL != originalBaseGainL,
            "Mix-only refresh should update pan-dependent base gain");
        Require(voice.attenuation < 1.0f,
            "Mix-only refresh should update attenuation");
        Require(NearlyEqual(voice.baseSampleStep, originalBaseSampleStep, 1.0e-12),
            "Mix-only refresh should not recompute base pitch state");
        Require(voice.sampleStepFixed == originalSampleStepFixed,
            "Mix-only refresh should not recompute sample step");
    ...
    void TestSf2MixAndPitchRefreshCanBeAppliedTogether() {
        MinimalSf2Config config;
        const std::vector<u8> bytes = BuildMinimalSf2(config);
        Sf2File sf2;
        Require(sf2.LoadFromMemory(bytes.data(), bytes.size()), sf2.ErrorMessage().c_str());
        ...
        Require(voice.baseGainL != originalBaseGainL,
            "Mix+Pitch refresh should update mix state");
        Require(voice.attenuation < 1.0f,
            "Mix+Pitch refresh should update attenuation");
        Require(NearlyEqual(voice.baseSampleStep / originalBaseSampleStep, 2.0, 1.0e-6),
            "Mix+Pitch refresh should update pitch state");
        Require(voice.sampleStepFixed != originalSampleStepFixed,
            "Mix+Pitch refresh should recompute sample step");
        Require(voice.filterBaseFcCents == originalFilterBaseFc,
            "Mix+Pitch refresh should not recompute filter state");
        Require(voice.filterEnabled == originalFilterEnabled,
            "Mix+Pitch refresh should preserve filter readiness");
        Require(voice.baseGainL != originalBaseGainL || voice.attenuation != originalAttenuation,
            "Mix+Pitch refresh should touch mix state without disturbing unrelated state");
    }

    void TestSf2MixAndFilterRefreshCanBeAppliedTogether() {
        MinimalSf2Config config;
        const std::vector<u8> bytes = BuildMinimalSf2(config);
        Sf2File sf2;
        Require(sf2.LoadFromMemory(bytes.data(), bytes.size()), sf2.ErrorMessage().c_str());
        ...
        Require(voice.baseGainL != originalBaseGainL,
            "Mix+Filter refresh should update mix state");
        Require(voice.attenuation < 1.0f,
            "Mix+Filter refresh should update attenuation");
        Require(voice.filterBaseFcCents == 9000,
            "Mix+Filter refresh should update filter cutoff");
        Require(voice.filterQCb == 300,
            "Mix+Filter refresh should update filter resonance");
        Require(NearlyEqual(voice.baseSampleStep, originalBaseSampleStep, 1.0e-12),
            "Mix+Filter refresh should not recompute base pitch state");
        Require(voice.sampleStepFixed == originalSampleStepFixed,
            "Mix+Filter refresh should not recompute sample step");
        Require(voice.baseGainL != originalBaseGainL || voice.attenuation != originalAttenuation,
            "Mix+Filter refresh should touch mix state without disturbing unrelated state");
        Require(voice.filterBaseFcCents != originalFilterBaseFc,
            "Mix+Filter refresh should touch filter state");
    }

    void TestSf2PitchAndFilterRefreshCanBeAppliedTogether() {
        MinimalSf2Config config;
        const std::vector<u8> bytes = BuildMinimalSf2(config);
        Sf2File sf2;
        Require(sf2.LoadFromMemory(bytes.data(), bytes.size()), sf2.ErrorMessage().c_str());
        ...
        Require(NearlyEqual(voice.baseSampleStep / originalBaseSampleStep, 2.0, 1.0e-6),
            "Pitch+Filter refresh should update pitch state");
        Require(voice.sampleStepFixed != originalSampleStepFixed,
            "Pitch+Filter refresh should recompute sample step");
        Require(voice.filterBaseFcCents == 9000,
            "Pitch+Filter refresh should update filter cutoff");
        Require(voice.filterQCb == 300,
            "Pitch+Filter refresh should update filter resonance");
        Require(voice.filterEnabled,
            "Pitch+Filter refresh should keep the filter path enabled for an active cutoff");
        Require(voice.baseGainL == originalBaseGainL,
            "Pitch+Filter refresh should not update pan-dependent base gain");
        Require(voice.attenuation == originalAttenuation,
            "Pitch+Filter refresh should not update attenuation");
    }

    void TestSf2MixEnvelopeAndLfoRefreshCanBeAppliedTogether() {
        MinimalSf2Config config;
        const std::vector<u8> bytes = BuildMinimalSf2(config);
        Sf2File sf2;
        Require(sf2.LoadFromMemory(bytes.data(), bytes.size()), sf2.ErrorMessage().c_str());
        ...
        Require(voice.baseGainL != originalBaseGainL,
            "Mix+Envelope+Lfo refresh should update mix state");
        Require(voice.attenuation < 1.0f,
            "Mix+Envelope+Lfo refresh should update attenuation");
        Require(voice.useModEnv,
            "Mix+Envelope+Lfo refresh should keep mod env enabled when ModEnvToPitch is present");
        Require(voice.modEnvToPitchCents == 900.0f,
            "Mix+Envelope+Lfo refresh should update ModEnvToPitch");
        Require(voice.modLfoDelayEnd > 0,
            "Mix+Envelope+Lfo refresh should update modulation LFO delay");
        Require(voice.modLfoPhaseStep > 0.0f,
            "Mix+Envelope+Lfo refresh should update modulation LFO rate");
        Require(voice.modLfoToPitchCents == 200.0f,
            "Mix+Envelope+Lfo refresh should update modulation LFO pitch depth");
        Require(voice.modLfoToFilterFcCents == 600.0f,
            "Mix+Envelope+Lfo refresh should update modulation LFO filter depth");
        Require(voice.modLfoToVolumeCb == 300.0f,
            "Mix+Envelope+Lfo refresh should update tremolo depth");
        Require(NearlyEqual(voice.baseSampleStep, originalBaseSampleStep, 1.0e-12),
            "Mix+Envelope+Lfo refresh should not recompute base pitch state");
        Require(voice.sampleStepFixed == originalSampleStepFixed,
            "Mix+Envelope+Lfo refresh should not recompute sample step");
        Require(voice.filterBaseFcCents == originalFilterBaseFc,
...
#define RUN_TEST(name) do { if (shouldRun(#name)) { g_currentTestName = #name; name(); } } while (0)
    ...
    RUN_TEST(TestSf2MixAndPitchRefreshCanBeAppliedTogether);
